...
// ####################################################################################################
// ################    VARS PROMOTED FROM main() TO FILE SCOPE FOR FUNCTION ACCESS    #################
// ####################################################################################################
...
// Game state
unsigned char rounds_completed;
FILE *fptr;
...
// Splash screens
unsigned char splash_screen_toggle;
...
unsigned loop_count_at_max_duration;

// Debug
unsigned int play_loop_time, play_loop_time_max;
...
unsigned char find_bunker_for_x(unsigned char right_margin)
{
    ...
    if (bn < 4)
    {
        bunkr_img_base_addr = (1 - active_player) * Bunkr_plyr1_img[bn]
                            + active_player * Bunkr_plyr2_img[bn];
    }
    return bn;
}
