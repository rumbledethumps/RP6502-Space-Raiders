...
unsigned char new_delay (ticks) {
    ...
    RIA.step1 = 0;
    v = RIA.vsync;
    for (k = 0; k < ticks; k++) {                   // # of ticks loop
        for (j = 0; j < 1; j++) {                   // NOT USED -- 1/60th of a second loop
            for (i = 0; i < 2548; i++) {            // watchdog timer ... loop until vsync is incremented
                if (v != RIA.vsync){
                    tempz = RIA.rw1 & 1;
                    if (!tempz) {
...
static void delay (unsigned tenths_of_seconds) {
    ...
    static uint8_t v;  
    v = RIA.vsync;
    for (k = 0; k < tenths_of_seconds; k++) {       // seconds loop
        for (j = 0; j < 1; j++) {                   // 1/60th of a second loop
            for (i = 0; i < 2548; i++) {            // loop until vsync is incremented
                if (v != RIA.vsync){
                    v = RIA.vsync;
                    break;
                }
...
unsigned char new_wait () {     // wait until key is pressed 
    unsigned char tempz;
...
unsigned char print_string (unsigned row, unsigned col, char * str, bool slow) {     // assumes no color change (text is white on black)
    bool skip = false;
...
static void erase_xram_sprite_config(void) {
    ...
    RIA.step0 = 1;
    for (i = 0; i < 48; i++) {  // clears 0x300 (48 * 16 = 768) bytes, actual sprite config data is just 0x250 bytes
...
void erase_top_of_bunkr2 (unsigned mem_start, unsigned char num_col, unsigned char lower_half) {
    ...
    while (i < num_col) {
        if (i%2 == 0) RIA.step1 = 64;
        else RIA.step1 = -64;
        RIA.rw1 = 0; RIA.rw1 = 0; RIA.rw1 = 0; RIA.rw1 = 0;    
        RIA.rw1 = 0; RIA.rw1 = 0; RIA.rw1 = 0; 
        RIA.step1 = 2; RIA.rw1 = 0;
        i++;
    }
}
...
void erase_bunkr_bullet_explos(unsigned mem_start) {
    ...
    while (i < 8) {
        ...
        else {  // reverse vertical direction, every other column
            RIA.step1 = -64;
            RIA.step0 = -16;            
        }
        if (RIA.rw0 > 0) RIA.rw1 = 0;
        else temp1 = RIA.rw1;
        if (RIA.rw0 > 0) RIA.rw1 = 0;
        else temp1 = RIA.rw1;        
        if (RIA.rw0 > 0) RIA.rw1 = 0;
        ...
        RIA.step0 = 2; RIA.step1 = 2;  // move to next column in memory, on next write
        if (RIA.rw0 > 0) RIA.rw1 = 0;
        else temp1 = RIA.rw1;
        i++;        
    }
...
unsigned char bunkr_bullet_micro_collision(unsigned mem_start) {    // find a pixel level collision
...
void erase_bunkr_bomb_explos(unsigned mem_start) {
    ...
    while (i < 6) {             // BULLET IMAGE starts at col 0 and has 6 columns, columns 7 & 8 are blank
        if (i%2 == 0) {
            RIA.step1 = 64;
            RIA.step0 = 16;
        } 
...
void restore_bunkers(unsigned char active_player) {
    ...
    if (active_player == 1) RIA.addr0 = BUNKR_0_PLYR2_IMG_BUF; 
    for (j = 0; j < 4; j++) {
...
void update_numerical_lives (bool blink_lives, unsigned char num_of_lives_remaining) {
    ...
    if (blink_lives) {
        ...
        if (active_player == 1) print_string (29, 34, lives_num_str, false);
        else print_string (29, 5, lives_num_str, false);        
    }
} 
...
void update_wave_number(bool top_bottom) {
    ...
    if (top_bottom) print_string (9, 15, level_num_str, false);   
    else print_string (29, 20, level_num_str, false); 
}
...
void update_score_board() {
    ...
    if (active_player == 1) print_string(2, 27, score_value_str, false);    // update 2nd player's score
    else print_string(2, 7, score_value_str, false);                        // update 1st player's score
...
unsigned char irq_fn(void) {
    ...
    if ((*via_ifr & 0xC0) > 0) {       // VIA interrupt occurred, bit 7 IRQB asserted, bit 6 Interrupt for Timer 1
...
void main()
{
    ...
    */
    ...
    while (1) { // Init Boot & Restart Game Loop 
        ...
        for (i = 0; i < 2; i++) {        
            for (j = 0; j < 55; j++) {
                if (i == 0) {
                    if (j < 22) Alien_img_ptr[i][j] = INVR_IMG_MAGENTAIE_0;
                    else if (j < 44) Alien_img_ptr[i][j] = INVR_IMG_BLUIE_0;
                    else Alien_img_ptr[i][j] = INVR_IMG_GREENIE_0;  
                }
                else {
                    if (j < 22) Alien_img_ptr[i][j] = INVR_IMG_MAGENTAIE_1;
                    else if (j < 44) Alien_img_ptr[i][j] = INVR_IMG_BLUIE_1;
                    else Alien_img_ptr[i][j] = INVR_IMG_GREENIE_1;  
                } 
            }                
        }
        ...
        if (Game.num_players == 1) another_player = true;
        ...
        // BOMB INITIALIZAITON
        for (i = 0; i < NUM_BOMB_SPR; i++) {       // i cycles thru bomb types, 0=screw/targeted, 1=spike, 2=sawtooth
            Bomb[i].exists = false;   
        ...
        ShotOverlapTop = 0; ShotOverlapBottom = 0, ShotColumn = 0;
        ...
        while (1) {
            ...
            if (round_is_over) { // either gunner was hit, alien landed or level was completed
                rounds_completed++;
                ...
                // ####################################                
                if (Gunner.blown_up) {  // s/b true when gunner is hit or alien has lannded 
                ...
                // blink new player's score start next round, set timers for spawning gunner, bombs and saucer                 
                    ...
                    if (active_player == 0) print_string (6, 23, "1>", !slow);
                    else print_string (6, 23, "2>", !slow);
                    ...
                    if (Player[active_player].index_start_y_pos > 8) Player[active_player].index_start_y_pos = 1;
                    ...
                    for (j = 0; j < 11; j++) Alien_unoccupied_rows_per_col[active_player][j] = 0;     
                    for (j = 0; j < 5; j++) Alien_unoccupied_cols_per_row[active_player][j] = 0;
                    alien_num = 54;
                    alien_x = alien_ref_x;
                    alien_y = alien_ref_y; 
                ...
                if (another_player) {            
                    ...
                    if (active_player == 0) print_string (6, 22, "1>", slow);
                    else print_string (6, 22, "2>", slow);
                    update_wave_number (false);                     
                    ...
                    for (i = 0; i < 4; i++) {   // switch to the other bank of 4 bunker images
                        ptr = SPR_CFG_BASE + ((BUNKR_FIRST_SPR_NUM + i) * sizeof(vga_mode4_sprite_t)); 
                        xram0_struct_set(ptr, vga_mode4_sprite_t, xram_sprite_ptr, bunkr_img_ptr);
            ...
            if (Player[active_player].bonus_active) {       // Handle BONUS case
                ptr = SPR_CFG_BASE + (LIVES_FIRST_SPR_NUM + ((Player[active_player].lives - 1) + (4 * active_player))) * (sizeof(vga_mode4_sprite_t));
            ...
            *via_ier = 0xC0;
            ...
            for (i = 0; i < 50; i++) max_duration_values[i] = 0;
            for (i = 0; i < 50; i+=2) {
                values[i] = 0;
                values[i+1] = 255;                
            } 
            ...
            while (1) {             
                ...
                if (RIA.vsync == v) continue;   // wait for blanking period (VSYNC increment)                    
                ...
                *via_count_h = 0xFF;
                ...
                if (skip_alien_sprite_update == false) {
                    ...
                    if (alien_explosion_done) {    // indicates termination process has been completed/is done
                ...
                if (alien_march_sfx_timer > 0) alien_march_sfx_timer--;
                // made it thru one pass of all alien sprites (all 55 are now visible)                    
                    ...
                    if (alien_march_note_sequ > 3) {
                        ...
                        if (toggle_tones == 1) frequency += 50;
                ...
                // ####  BULLET SPAWN/RELOAD  #### 
                if (Bullet.reload == 1 && Gunner.exists && !Gunner.hit) {      // if so, load bullet into gunner
                    ...
                    if ((Gunner.shoot && Game.play_mode) || !Game.play_mode) {  
                        ...
                        if (Player[active_player].bullets_fired > 14) Player[active_player].bullets_fired = 0;
                        Bullet.reload = 0;
                        // SFX
                        loops = 0;
                        bullet_loops = 0;
                ...
                if(bullet_hit == 0) {                      
                    if(Bullet.exists) {    // inflight and not exploding, so continue to move it until it collides and is terminated
                        ...
                        if ((bullet_loops > 4) && true) {
                            if (bullet_loops == 10) bullet_freq = 2000;
                            if (bullet_loops == 5) bullet_freq = 1000;
                            bullet_loops++;
                            if (bullet_loops == 15) bullet_loops = 5;
                            // load frequ
                            RIA.addr0 = BULLET_SFX_BASE_ADDR;       
                            RIA.step0 = 1;   
                            RIA.rw0 = bullet_freq & 0xFF;
                ...
                // ###############################################
                if (bullet_hit == 0) {
                    for (i = 0; i < 3; i++) {       // loop through all bombs
                        if (Bomb[i].exists && (Bomb[i].hit == 0)) {
                            if (bullet_x0 >= Bomb[i].x0 && bullet_x0 < Bomb[i].x1) {
                                ...
                                if (ShotOverlapBottom > 0 && ShotOverlapTop > 0) {  // then bullet overlaps bomb in y axis
                                    if (ShotOverlapTop < 4) {                                       
                                        BombTopRow = 0;
                ...
                if ((bullet_hit == 0) && !bypass_test_mode) {
                    ...
                    if ((delta_y > 0) && (delta_y <= 16)) {   // Macro check for y-axis 
                        ...
                        if ((delta_x >= 0) && (delta_x < 45 + 45 + 45 + 22)) {   
                            ...
                            if (delta_x >= 45 + 45) {   // left or right side of (approx) midpoint of the 4 bunkers?
                                if (delta_x >= 45 + 45 + 45) {
                                    bunkr_num = 3;                               
                ...
                if ((bullet_hit == 0) && (delta_y >= 4) && (delta_y <= (64 + 12))) {     // 8 to account for alien x1
                    ...
                    if (delta_x >= 0 && delta_x < INVR_MTRX_WIDTH + 16) {
                        ...
                        if (Players_Alien_Exists[active_player][num_of_alien_hit] == 1) {                        
                            ...
                            if (Alien_update[num_of_alien_hit] != alien_ref_update ) {
                                ...
                                if (Player[active_player].alien_x_incr == +2) {               // moving aliens to the right, offset bbox to the left by 2
                            ...
                            else {  // hit alien has already been updated to match reference pos
                            ...
                            if (alien_hit == 1) {
                                ...
                                if (num_of_alien_hit < 21) Player[active_player].score+=1;  
                                else if (num_of_alien_hit < 43) Player[active_player].score+=2;
                                else Player[active_player].score+=3;                                
                                if (true) {
                ...
                // ##########################
                if (Saucer.score_start_time > 0) Saucer.score_start_time--;
                if (Saucer.score_start_time == 1) { // since it's a one time event, need to use value = 1 (not 0)
                ...
                if (bullet_hit == 0 && Saucer.exists) {                    
                    if (bullet_y < (SAUCER_BASE_Y + SAUCER_BBOX_Y1)) {
                        if (bullet_y1 >= (SAUCER_BASE_Y + SAUCER_BBOX_Y0)) {
                            if (bullet_x0 >= (Saucer.x + SAUCER_BBOX_X0)) { // -3 to account for bullet sprite offset from x by +3 pixels 
                ...
                if (Game.bomb_spawn_time > 0) Game.bomb_spawn_time -= 1;    // counts down, stops at zero
                ...
                if (Player[active_player].num_of_aliens < 9) bomb_speed = 5; 
                
                // #############
                // SCREW BOMB 0
                // #############                   
                if (bomb_type_counter == 0) {       // run screw handler, if count = zero and "skip" if false
                    bomb_type_counter = 2;          // since reached zero, reset bomb type counter to starting count = 2                         
                    // ###  MOVE  ###
                    if (Bomb[0].exists == true) {  
                        if (Bomb[0].hit == 0) {     // if not exploding, move & animate it                         
                            Bomb[0].y += bomb_speed;
                            Bomb[0].y0 = Bomb[0].y + BOMB_BBOX_SCREW_Y0;
                            Bomb[0].y1 = Bomb[0].y + BOMB_BBOX_Y1;
                            Bomb[0].numbr_steps_taken++;
                            Bomb[0].anim_sequ_num += 1;
                            if (Bomb[0].anim_sequ_num > 3) Bomb[0].anim_sequ_num = 0;
                            bomb_num_just_updated = 0;                  
                        }
                    } 
                    else { // bomb doesn't exist so SPAWN one
                    // ###  SPAWN  ###
                    // Define screw sprite pos, image/anim sequ, update steps, inidcate existence 
                        if (bomb_screw_skip == 0) {     // don't skip the first cycle, but every other cycle after that                           
                            if ((Bomb[1].numbr_steps_taken == 0) || (Bomb[1].numbr_steps_taken > bomb_reload_rate)) {
                                if ((Bomb[2].numbr_steps_taken == 0) || (Bomb[2].numbr_steps_taken > bomb_reload_rate)) { 
                                    if(Game.bomb_spawn_enable && (Player[active_player].num_of_aliens > 0) && (Gunner.exists) && !Gunner.blown_up) {
                                        ...
                                        if (alien_col_num < 11) {
                                            if (Alien_unoccupied_rows_per_col[active_player][alien_col_num] < 5) { // valid col # (>0n & <11) and col is occupied
                        ...
                        else bomb_screw_skip = 0; // this is set to 1 when screw bomb terminates to prevent immediately respawn
                    }
                }
                else bomb_type_counter -= 1;  // decrement if not zero
                ...
                // ##############
                // BOMB 1 MOVE/SPAWN
                if (bomb_type_selector == 1) {  // run spike handler
                    // ### MOVE ###
                    if (Bomb[1].exists == true) {
                        if (Bomb[1].hit == 0) {
                            ...
                            if (Bomb[1].anim_sequ_num > 3) Bomb[1].anim_sequ_num = 0;
                        ...
                        if (Bomb[0].numbr_steps_taken == 0 || (Bomb[0].numbr_steps_taken > bomb_reload_rate)) {
                            if (Bomb[2].numbr_steps_taken == 0 || (Bomb[2].numbr_steps_taken > bomb_reload_rate)) {
                                ...
                                if(Game.bomb_spawn_enable && (Player[active_player].num_of_aliens > 1) && (Gunner.exists) && !Gunner.blown_up) {                                 
                                    ...
                                    if (Alien_unoccupied_rows_per_col[active_player][alien_col_num] < 5) { // valid col # (>0n & <11) and col is occupied                                           
                                        for (i = 0; i < 5; i++) {   // check if column is occupied, starting at bottom and working up
                                            if ((Players_Alien_Exists[active_player][alien_num_to_drop_bomb_from] == 0) || 
                                                ((alien_hit > 0) && (num_of_alien_hit == alien_num_to_drop_bomb_from))) {
                                                alien_num_to_drop_bomb_from += 11;
                ...
                // SAWTOOTH BOMB
                // #################                
                if (bomb_type_selector == 2) {  // run sawtooth handler
                    // ### MOVE ###
                    if (Bomb[2].exists == true) {
                        if (Bomb[2].hit == 0) {
                            ...
                            if (Bomb[2].anim_sequ_num > 3) Bomb[2].anim_sequ_num = 0;
                        ...
                        if (Bomb[0].numbr_steps_taken == 0 || (Bomb[0].numbr_steps_taken > bomb_reload_rate)) {
                            if (Bomb[1].numbr_steps_taken == 0 || (Bomb[1].numbr_steps_taken > bomb_reload_rate)) {
                                if(Game.bomb_spawn_enable && (Gunner.exists) && !Gunner.blown_up) {                                 
                                    ...
                                    alien_num_to_drop_bomb_from = alien_col_num;
                                    if (++Player[active_player].col_index_sawtooth > 15) Player[active_player].col_index_sawtooth = 0; 
                                    // starts 1 px higher than others, since it's 1 px shorter 
                                    Bomb[2].drop_rel_y = alien_ref_y + 21;    // +17 to start drop 5 px below bottom edge of lowest alien                                    
                                    if (Alien_unoccupied_rows_per_col[active_player][alien_num_to_drop_bomb_from] < 5) { // valid col # (>0n & <11) and col is occupied                                           
                                        for (i = 0; i < 5; i++) {   // check if column is occupied, starting at bottom and working up
                                            if ((Players_Alien_Exists[active_player][alien_num_to_drop_bomb_from] == 0) || 
                                                ((alien_hit > 0) && (num_of_alien_hit == alien_num_to_drop_bomb_from))) {
                                                alien_num_to_drop_bomb_from += 11;
                ...
                if((Bomb[bomb_num_just_updated].hit != 4) && !bypass_test_mode) {
                    ...
                    if ((delta_y > 0) && (delta_y <= 16 + 4)) {   // Macro check for y-axis, using +4 to allow hit when 1/2 of bomb is non-overlapping at bottom of bunker
                        // Macro test x-axis where do we have overlap, if any?                  
                        ...
                        if ((delta_x > 2) && (delta_x < 45 + 45 + 45 + 27)) {  
                            // x-axis binary search for overlap with bunker, if any
                            bunkr_num = 4;  // default value indicates no alignment with any bunker                        
                            if (delta_x > 45 + 45 + 2) {   // is bomb on the left or right side of (approx) midpoint of the 4 bunkers?
                ...
                if (!Gunner.hit) {
                    ...
                    if ((Bomb[bomb_num_just_updated].hit == 6) || alien_landed) { // round has ended, do final "end of round" clean up
                        Gunner.hit = true;
                        if (Player[active_player].lives > 0) Player[active_player].lives--;
                        update_numerical_lives(!blink, Player[active_player].lives);
                ...
                if (Bullet.explos_ticks > 0) Bullet.explos_ticks--;
                if (bullet_hit > 0) {                  
                    if (bullet_hit_subset2 > 0) {
                        if (Player[active_player].score > 1229 ) bomb_reload_rate = 7;
                        else if (Player[active_player].score > 819 ) bomb_reload_rate = 8;
                        else if (Player[active_player].score > 409 ) bomb_reload_rate = 11;
                        else if (Player[active_player].score > 51) bomb_reload_rate = 16;
                        else bomb_reload_rate = 48;
                    }
                    if (Bullet.explos_ticks == 0) {
                        if (bullet_hit_subset2 > 0) update_score_board();                          
                        // turn off SFX
                        if (alien_hit == 1) {
                            RIA.addr0 = BULLET_SFX_BASE_ADDR + PAN_GATE;
                            RIA.rw0 = 0;
...
// ... even though it is not there (doesn't exist)                            
                        ...
                        if (bullet_saucer_hit == 1 && 1) {
                            if (frequency <= 500) ramp_up = true;
                            if (frequency > 2500) ramp_up = false;
                            if (ramp_up) frequency += 1564;
                            else frequency -= 1564; 
                            // load frequ            
                ...
                // #########################
                if (alien_explos_ticks > 0) alien_explos_ticks--;
                if (alien_hit == 1) { 
                ...
                // ##########################
                for (i = 0; i < 3; i++) {
                    if (Bomb[i].explos_ticks > 0) Bomb[i].explos_ticks--;   
                    if ((Bomb[i].hit > 0) && (Bomb[i].explos_ticks == 0)) {  // bomb explosion is done, terminate
                        // must wait one cycle after screw bomb terminates before enabling respawn
                        if (i == 0) bomb_screw_skip = 1;  
                ...
                // ############################
                if(Gunner.explos_ticks > 0) Gunner.explos_ticks--;
                if (Gunner.hit) {
                    if(Gunner.explos_ticks != 0) {                         
                        ...
                        if (active_player == 1) gunner_image_ptr = GUNNER_PLYR2_IMG_BASE + SPR_16X16_SIZE;                      
                        ...
                        if (Gunner.explos_ticks > GUNNER_EXPL_TICKS - 5) RIA.rw0 = ((uint8_t)((rand()%(0xF0 - 0x08 + 1)) + 0x08) & 0xFF); // byte 2 = duty cycle
                        else dummy_read = RIA.rw0;                          // to increment addr either way 
                    ...
                    else {  // gunner explosion is complete, time to process end end of this round                      
                        ...
                        Gunner.y = DISAPPEAR_Y;         // disappear gunner/explosion
                        gunner_image_ptr = GUNNER_PLYR1_IMG_BASE;
                        if (active_player == 1) gunner_image_ptr = GUNNER_PLYR2_IMG_BASE; 
                    } 
                }
                ...
                if(Gunner.exists) {
                    if (!Gunner.hit) {
                        ...
                        if (!Game.play_mode) {
                            if (rand() %96 == 0) gunner_demo_direction_right = !gunner_demo_direction_right;
                            ...
                            if (gunner_demo_direction_right == true) Gunner.x+= GUNNER_SPEED; 
                            else Gunner.x-= GUNNER_SPEED;        
                        }  
                        // #####   PLAY MODE   #####
                        // based on player input
                        else if (Game.play_mode) {
                            if (Gunner.direction_right && Gunner.x <= 249) Gunner.x += GUNNER_SPEED;
                            if (Gunner.x > 249) {     
                                Gunner.x = 249;             
                            }
                            if (Gunner.direction_left && Gunner.x >= 55) Gunner.x -= GUNNER_SPEED;
                            if (Gunner.x < 55) {
                                Gunner.x = 55;                                   
                            } 
                            Gunner.direction_right = false;                
                ...
                // ####  GUNNER TIMER UPDATE  ####
                if (Gunner.spawn_time > 0) Gunner.spawn_time -= 1;  // decrement if timer has been started (not zero)
                ...
                // ####  UPDATE SPAWN TIMER  ####
                if (Saucer.next_spawn_time > 0) Saucer.next_spawn_time--;
                ...
                if (Saucer.exists && !level_completed && true) {               
                    if (Saucer.left == 1) Saucer.x -= SAUCER_SPEED; // move saucer one px left or right depending on direction flag
                    else Saucer.x += SAUCER_SPEED;                    
                    // do SFX
                    RIA.addr0 = SAUCER_SFX_BASE_ADDR + PAN_GATE;   // PUSH PLAY                
                    RIA.rw0 = 1 & 1;
                    // do frequency ramp up then down, one frequency step per tick
                    if (frequency <= 1235) ramp_up = true;
                    if (frequency > (1235 + (6 * 2127))) ramp_up = false;
                    if (ramp_up) frequency += 1127;
                    else frequency -= 1127; 
                    // load frequ            
                    RIA.addr0 = SAUCER_SFX_BASE_ADDR;       
                    RIA.step0 = 1;  
                ...
                if (!Saucer.exists && Saucer.spawn_enable) {
                    ...
                    if (Saucer.left == 1) Saucer.x = 319-48;        // start off screen on the selected side
                    else Saucer.x = 47 - 16;
                ...
                if(!((alien_hit == 1) || Gunner.hit || Gunner.blown_up || level_completed)) {                    
                    ...
                    while (Players_Alien_Exists[active_player][alien_num] == 0) {  
                        ...
                        if (alien_num > 54) {       // handle 'end/start of matrix" calculations
                            alien_num = 0;
                            alien_roll_over = 1; 
                    ...
                    if (alien_roll_over == 1) {
                        s++;
                        if (alien_march_sfx_start < 3) alien_march_sfx_start++;
                        // reset flags 
                        alien_roll_over = 0;
                        alien_drop = 0;                                             
                        ...
                        if (Player[active_player].num_of_aliens == 1) {
                            ...
                            else if (alien_drop == 1) {  // should be heading left
                    ...
                    if (alien_y > 200) alien_landed = true;
                }  // END ALIEN CODE
                ...
                if ((Alien_rel_y[alien_num] + alien_ref_y + 4) >= (BUNKR_Y + 8) && 
                    ...
                    if ((delta_x > 0) && (delta_x < 45 + 45 + 45 + 22 + Alien_width[alien_num])) {   
                        ...
                        if (bunkr_num < 4) {                      
                            ...
                            lower_half_bunkr = 0;
                            if((Alien_rel_y[alien_num] + alien_ref_y + 4) >= BUNKR_Y + 16) lower_half_bunkr = 1;
